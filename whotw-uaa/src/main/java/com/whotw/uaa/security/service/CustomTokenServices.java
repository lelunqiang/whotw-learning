...
public class CustomTokenServices extends DefaultTokenServices {
    ...
    public OAuth2AccessToken refreshAccessToken(String refreshTokenValue, TokenRequest tokenRequest)
        ...
        if (this.authenticationManager != null && !authentication.isClientOnly()) {
            ...
            //定制：这里如果是WechatAuthenticationToken就不必去走用户名密码校验了，而是直接丢给provider去检查是否过期
            Authentication user = authentication.getUserAuthentication();
            if (!(authentication.getUserAuthentication() instanceof CustomAuthenticationToken))
                user = new PreAuthenticatedAuthenticationToken(authentication.getUserAuthentication(), "", authentication.getAuthorities());
    ...
    private OAuth2AccessToken createAccessToken(OAuth2Authentication authentication, OAuth2RefreshToken refreshToken) {
        ...
        OAuth2AccessToken resultToken = accessTokenEnhancer != null ? accessTokenEnhancer.enhance(token, authentication) : token;
        Authentication userAuthentication = authentication.getUserAuthentication();
        if(userAuthentication!=null && !(userAuthentication instanceof WechatAuthenticationToken)){
            String mobile = (String) authentication.getUserAuthentication().getPrincipal();
            //记录每次登陆数据，主由mobile,jwtToken后8位构成
            //token通过jwt转化为唯一的jwtToken，jwtToken第三部分唯一，取后八位即可]
            String tokenValue = resultToken.getValue();
            tokenValue = tokenValue.substring(tokenValue.length() - 8);
            redisTemplate.opsForValue().set(getKeySign(mobile) + tokenValue, 1, accessTokenValiditySeconds, TimeUnit.SECONDS);
        }
        return resultToken;

    }

    private String getKeySign(String mobile) {
        return SecurityConstants.LOGIN +
                Constants.CHAR_DOUBLE_COLON +
                mobile +
                Constants.CHAR_UNDER_LINE;
    }
